...
private var enemyType:environment.EnemyTypeEnum;
...
function Update () {
	...
	if(counter > spawnRate){
		...
		var cloneEnemy1:GameObject;
		cloneEnemy1 = Instantiate(enemy1, Vector3(transform.position.x,0,Random.Range(minHeight,maxHeight)), Quaternion.Euler(-90,0,0));
		enemyType = environment.EnemyTypeEnum.Asteroid;
		AddToEnv(cloneEnemy1,enemyType);
		}
		//if the random number ends up being 4, it'll spawn the orb, set as enemy2
		if(randomChoice == 4){
		var cloneEnemy2:GameObject;
		cloneEnemy2 = Instantiate(enemy2, Vector3(transform.position.x,0,Random.Range(minHeight/3,maxHeight/3)), Quaternion.Euler(-90,0,0));
		enemyType = environment.EnemyTypeEnum.Orb;
		AddToEnv(cloneEnemy2,enemyType);
		}
		//if the random number ends up being 5, it'll spawn the ship, set as enemy3
		if(randomChoice == 5){
		var cloneEnemy3:GameObject;
		cloneEnemy3 = Instantiate(enemy3, Vector3(transform.position.x,0,Random.Range(minHeight,maxHeight)), Quaternion.Euler(-90,90,0));
		enemyType = environment.EnemyTypeEnum.EnemyShip;
		AddToEnv(cloneEnemy3,enemyType);
		}
		counter = 0.0;
	}
}

function AddToEnv(object:GameObject,enemyType:environment.EnemyTypeEnum)
{
var gui:GameObject = GameObject.Find("GUI");
var env:environment = gui.GetComponent(environment);
env.addEnemy(object,enemyType);
}